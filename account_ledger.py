...
class FinanceAccount(db.Model):
    ...
    def pending_change(self):
        return sum(float(x.amount or 0) for x in self.incoming_transactions if x.status=="Pending Confirmation")-sum(float(x.amount or 0) for x in self.outgoing_transactions if x.status=="Pending Confirmation")
...
def _validate_source(coop_id,source_type,source_id):
    ...
    if not source_type or not source_id: abort(400,"Source type and source ID must be supplied together.")
    model_name=SOURCE_MODELS.get(source_type)
    if not model_name: abort(400,"Choose a supported source record type.")
    model=getattr(_core,model_name,None)
    if model is None: abort(400,"Source model is unavailable.")
    row=model.query.filter_by(id=source_id,cooperative_id=coop_id).first()
    if row is None: abort(400,"Source record does not belong to this cooperative or does not exist.")
...
def transaction_decision(item_id):
    ...
    if row.recorded_by_user_id==session.get("user_id"): return "You cannot approve or reject a transaction you recorded.",403
