...
class LedgerTransaction(db.Model):
    ...
    created_at=db.Column(db.DateTime,nullable=False,default=utc_now)
    cooperative=db.relationship("Cooperative",foreign_keys=[cooperative_id])
    from_account=db.relationship("FinanceAccount",foreign_keys=[from_account_id],
        backref=db.backref("outgoing_transactions",lazy=True))
    to_account=db.relationship("FinanceAccount",foreign_keys=[to_account_id],
...
def _notify(coop_id,roles,title,message,source_type,source_id,severity="Info"):
    for access in UserAccess.query.filter(UserAccess.cooperative_id==coop_id,UserAccess.status=="Active",
                                          UserAccess.role.in_(tuple(roles))).all():
        _upsert_notification(access.user_id,coop_id,"Finance",title,message,severity,url_for("ledger.dashboard"),
...
def dashboard():
    ...
    transactions=LedgerTransaction.query.filter_by(cooperative_id=coop.id).order_by(
        LedgerTransaction.transaction_date.desc(),LedgerTransaction.created_at.desc()).limit(100).all()
    confirmed_total=sum(a.confirmed_balance for a in accounts)
    pending_total=sum(a.pending_change for a in accounts)
    return render_template("finance_accounts/dashboard.html",accounts=accounts,transactions=transactions,
                           confirmed_total=confirmed_total,pending_total=pending_total,
                           account_types=ACCOUNT_TYPES,income_categories=INCOME_CATEGORIES,
...
def account_create():
    ...
    if FinanceAccount.query.filter(func.lower(FinanceAccount.name)==name.lower(),FinanceAccount.cooperative_id==coop.id).first():
        return "An account with this name already exists.",400
    item=FinanceAccount(cooperative_id=coop.id,name=name[:120],account_type=account_type,
                        institution=request.form.get("institution","").strip()[:120] or None,
                        account_last4=request.form.get("account_last4","").strip()[-4:] or None,
                        opening_balance=float(opening),notes=request.form.get("notes","").strip() or None,
                        created_by_user_id=session["user_id"])
    db.session.add(item);db.session.flush()
    add_audit_log("FINANCE_ACCOUNT_CREATED","FinanceAccount",item.id,item.name,cooperative_id=coop.id)
...
def transaction_create():
    ...
    from_id=parse_int(request.form.get("from_account_id"));to_id=parse_int(request.form.get("to_account_id"))
    ...
    if kind in {"Income","Opening Balance"}:
        ...
        if not from_id or not to_id or from_id==to_id or from_id not in owned or to_id not in owned:
            return "Choose two different accounts in this cooperative for a transfer.",400
    category=request.form.get("category","").strip() or kind
    row=LedgerTransaction(cooperative_id=coop.id,transaction_type=kind,category=category[:100],amount=float(amount),
                          transaction_date=transaction_date,from_account_id=from_id,to_account_id=to_id,
                          counterparty=request.form.get("counterparty","").strip()[:180] or None,
                          reference=request.form.get("reference","").strip()[:120] or None,
                          source_type=request.form.get("source_type","").strip()[:60] or None,
                          source_id=parse_int(request.form.get("source_id")),status="Pending Confirmation",
                          notes=request.form.get("notes","").strip() or None,recorded_by_user_id=session["user_id"])
    db.session.add(row);db.session.flush()
    _notify(coop.id,FINANCE_APPROVAL_ROLES,"Account transaction awaiting approval",
            f"{kind}: R{amount:.2f} — {category}.","LedgerTransactionApproval",row.id,"Warning")
    add_audit_log("LEDGER_TRANSACTION_RECORDED","LedgerTransaction",row.id,f"{kind} R{amount:.2f}",cooperative_id=coop.id)
    db.session.commit();flash("Transaction recorded for Chairperson confirmation.","success");return redirect(url_for("ledger.dashboard"))
...
def transaction_decision(item_id):
    ...
    if decision=="approve" and row.transaction_type in {"Expense","Transfer"}:
        if row.from_account.confirmed_balance+1e-9<float(row.amount):
    ...
    _notify(coop.id,FINANCE_RECORD_ROLES,f"Account transaction {row.status.lower()}",
            f"{row.transaction_type} R{row.amount:.2f} ({row.category}) was {row.status.lower()}.",
            "LedgerTransactionDecision",row.id,"Info" if decision=="approve" else "Warning")
    add_audit_log("LEDGER_TRANSACTION_CONFIRMED" if decision=="approve" else "LEDGER_TRANSACTION_REJECTED",
                  "LedgerTransaction",row.id,f"{row.transaction_type} R{row.amount:.2f}",cooperative_id=coop.id)
    db.session.commit();return redirect(url_for("ledger.dashboard"))
